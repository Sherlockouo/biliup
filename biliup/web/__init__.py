...
from biliup.database.models import (
    UploadStreamers,
    LiveStreamers,
    Configuration,
    StreamerInfo,
...
def check_similar_remark(json_data):
    ...
    for fname in config["streamers"].keys():
        if json_data["remark"] in fname or fname in json_data["remark"]:
            return True
    return False
            ...
            file_list.append(
                {
                    "key": i,
                    "name": file_name,
                    "updateTime": os.path.getmtime(file_name),
...
async def streamers(request):
    ...
    with SessionLocal() as db:
        ...
        for ls in result:
            ...
            if temp.get(
                "upload_streamers_id"
            ):  # 返回 upload_id 而不是 upload_streamers
...
async def add_lives(request):
    ...
    if check_similar_remark(json_data):
        return web.HTTPBadRequest(
            text=f"{json_data['remark']} 与现存备注存在部分重复，禁止添加"
        )
    uid = json_data.get("upload_id")
    with SessionLocal() as db:
        if uid:
            us = db.get(UploadStreamers, uid)
            to_save = LiveStreamers(
                **LiveStreamers.filter_parameters(json_data), upload_streamers_id=us.id
            )
        else:
...
async def lives(request):
    ...
    with SessionLocal() as db:
        ...
        if json_data["remark"] != old.remark:
            if check_similar_remark(json_data):
                return web.HTTPBadRequest(
                    text=f"{json_data['remark']} 与现存备注存在部分重复，禁止修改"
                )
        uid = json_data.get("upload_id")
        ...
        new_data = {
            ...
            for key in old.as_dict()
            if key != "upload_streamers_id"
        }
        try:
            if uid:
                ...
                db.execute(
                    update(LiveStreamers), [{**new_data, "upload_streamers_id": us.id}]
                )
                db.commit()
            else:
...
async def users(request):
    try:
        ...
        return web.json_response(
            {"status": 500, "error": f"有多个空间配置同时存在: {e}"}, status=500
        )
...
async def m_upload(request):
    ...
    threading.Thread(
        target=biliup_uploader, args=(json_data["files"], json_data["params"])
    ).start()
...
async def myinfo(request):
    ...
    try:
        ...
        return web.json_response(
            {"status": 500, "error": f"{file} 文件不存在"}, status=500
        )
    cookies = config.data["user"]["cookies"]
...
def find_all_folders(directory):
    ...
    for foldername, subfolders, filenames in os.walk(directory):
        for subfolder in subfolders:
            result.append(
                os.path.relpath(os.path.join(foldername, subfolder), directory)
            )
    return result
...
async def service(args):
    ...
    if args.static_dir:
        ...
        res = []
        for fdir in pathlib.Path(files("biliup.web").joinpath("public")).glob(
            "**/*.html"
        ):
            ...
            def _copy(fname):
                async def static_view(request):
                    return web.FileResponse(
                        files("biliup.web").joinpath("public").joinpath(fname)
                    )
            ...
            res.append(
                web.get(
                    "/" + str(fname.with_suffix("")).replace("\\", "/"), _copy(fname)
                )
            )
...
async def handle_500(request):
    return web.json_response(
        {"status": 500, "error": "Error handling request"}, status=500
    )
